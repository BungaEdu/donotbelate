...
class TtsManager() {
    ...
    private fun configureLanguage(context: Context) {
        try {
            ...
            reportCrash(TAG, "No se ha podido recoger el idioma correctamente: $e")
        } catch (e: Exception) {
            reportCrash(TAG, "Error inesperado al intentar coger el lenguaje del dispositivo : $e")
        }
    }
    ...
    fun off() {
        ...
        textToSpeech?.let {
            try {
                ...
                reportCrash(TAG, "Error occurred while shutting down TextToSpeech: $e")
            }
        }
        textToSpeech = null
    }
    ...
    private fun getLocaleForLanguage(language: String): Locale {
        // TODO Implementación del método para obtener el Locale según el idioma
